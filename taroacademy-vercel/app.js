...
function applyRoute() {
  ...
  if (!studentPage || !adminPage) return;

  studentPage.classList.toggle("hidden", isAdmin);
  adminPage.classList.toggle("hidden", !isAdmin);
  if (brandTitle) brandTitle.textContent = isAdmin ? "TaroAcademy CRM" : "TaroAcademy";
  if (brandSubtitle) brandSubtitle.textContent = isAdmin ? "Клиенти и видео уроци" : "Зона за видео уроци";
  if (studentLink) studentLink.classList.toggle("hidden", !isAdmin);
}
...
function setupAdminPage() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (document.getElementById("adminPin").value !== ADMIN_PIN) {
      document.getElementById("adminError").textContent = "PIN кодът не е правилен.";
      return;
    }
